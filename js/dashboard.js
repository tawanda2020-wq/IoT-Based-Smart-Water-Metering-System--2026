...
const FIREBASE_CONFIG = {
    ...
    databaseURL:       "https://iot-smart-h20-metering-system-default-rtdb.firebaseio.com/",
...
class WaterDashboard {
    constructor() {
        ...
        this.chartBuffer = {
            labels: [], srcFlow: [], dstFlow: [],
            srcTotal: [], dstTotal: [], maxPoints: 60,
        };
    ...
    handleLogout() {
        ...
        if (this.unsubSensor) { this.unsubSensor(); this.unsubSensor = null; }
    ...
    initFirebase() {
        ...
        this.alertsRef.on("value", (snap) => {
            const raw = snap.val();
            if (!raw) { this.alerts = []; this.renderAlerts(); return; }
    ...
    handleSensorData(data) {
        ...
        this.setText("totalConsumptionValue",
            ((data.sourceTotalLiters || 0) + (data.destTotalLiters || 0)).toFixed(1));
        this.setText("flowDifferentialValue",(data.flowDifferential || 0).toFixed(2));
        this.setText("literDifferenceValue", (data.literDifference  || 0).toFixed(1));
        ...
        if (data.pumpESP32Connected) {
            ...
            if (badge) { badge.textContent = "Pump Ctrl Offline"; badge.className = "kpi-badge offline"; }
    ...
    async controlPump(action) {
        if (action === "on" && this.currentData?.leakDetected) {
            if (!confirm("⚠️ A leak was detected. Start pump anyway?\nThis may waste water."))
                return;
        }
        ...
        try {
            ...
            this.toast(`✅ Pump ${action.toUpperCase()} command sent - awaiting hardware`, "success");
        } catch (err) {
    ...
    maybeCreateLeakAlert(data) {
        // Avoid flooding - check if there's already an active leak alert
        ...
        const alertObj = {
            ...
            type:      "leak",
            title:     "Leak Detected",
            message:   `Flow collapsed - dest: ${(data.destFlow || 0).toFixed(2)} L/min`,
            severity:  "high",
    ...
    renderAlerts() {
        ...
        if (this.alerts.length === 0) {
            list.innerHTML = '<div class="no-alerts">✅ No active alerts - system operating normally</div>';
            return;
        }

        list.innerHTML = [...active, ...resolved].map(a => `
    ...
    // ══════════════════════════════════════════════════
    logDailyData(data) {
        // Store today's latest totals under /dailyLog/YYYY-MM-DD
        const today = new Date().toISOString().split("T")[0];
        this.dailyLogRef.child(today).set({
            date:      today,
            supplied:  parseFloat((data.sourceTotalLiters || 0).toFixed(1)),
            delivered: parseFloat((data.destTotalLiters   || 0).toFixed(1)),
            loss:      parseFloat((data.literDifference   || 0).toFixed(1)),
    ...
    updateLeakState(data) {
        ...
        if (data.leakDetected) {
            ...
            if (diffBadge) { diffBadge.textContent = "LEAK DETECTED"; diffBadge.className = "kpi-badge offline"; }
            ...
            if (diffBadge) { diffBadge.textContent = "Potential Leak"; diffBadge.className = "kpi-badge warning"; }
            if (countdown) countdown.style.display = "block";
            this.startLeakCountdown(data.consecutiveLeakReadings);
        } else {
            if (diffCard) diffCard.classList.remove("leak-alert");
            if (srcCard)  srcCard.classList.remove("leak-alert");
            if (diffBadge) { diffBadge.textContent = "Normal"; diffBadge.className = "kpi-badge success"; }
            if (countdown) countdown.style.display = "none";
            this.stopLeakCountdown();
        }
    }
    ...
    updateKPICardColors(data) {
        ...
        if (srcBadge) {
            ...
            srcBadge.className   = srcFlow > 0.1 ? "kpi-badge success" : "kpi-badge offline";
        }
        if (dstBadge) {
            dstBadge.textContent = dstFlow > 0.1 ? "Active" : "No Flow";
            dstBadge.className   = dstFlow > 0.1 ? "kpi-badge success" : "kpi-badge offline";
        }
    }
    ...
    updatePumpUI(isOn) {
        ...
        [indicator, badge].forEach(el => { if (el) el.className = `conn-indicator ${cls}`; });
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        ...
                        bodyColor: "#f1f5f9", borderColor: "rgba(255,255,255,0.08)", borderWidth: 1,
                    },
                },
                scales: {
                    x: {
                    ...
                    y: {
                        ...
                        title: { display: true, text: "L/min", color: "#475569", font: { size: 10 } },
                    },
                    y1: {
                        position: "right",
                        grid: { drawOnChartArea: false },
                        ticks: { color: "#475569", font: { size: 10 } },
                        title: { display: true, text: "Litres (total)", color: "#475569", font: { size: 10 } },
                    },
                },
            },
        });
    ...
    pushChartPoint(data, time) {
        ...
        if (b.labels.length > b.maxPoints) {
            b.labels.shift(); b.srcFlow.shift(); b.dstFlow.shift();
            b.srcTotal.shift(); b.dstTotal.shift();
        }
    ...
    clearChartBuffer() {
        const b = this.chartBuffer;
        b.labels = []; b.srcFlow = []; b.dstFlow = []; b.srcTotal = []; b.dstTotal = [];
        if (this.flowChart) {
    ...
    initConsumptionChart() {
        ...
        this.consumptionChart = new Chart(ctx.getContext("2d"), {
            ...
            data: {
                labels: [],
                datasets: [
                    { label: "Supplied (L)",  data: [], backgroundColor: "rgba(56,189,248,0.6)", borderColor: "#38bdf8", borderWidth: 1 },
                    { label: "Delivered (L)", data: [], backgroundColor: "rgba(52,211,153,0.6)", borderColor: "#34d399", borderWidth: 1 },
                    { label: "Loss (L)",      data: [], backgroundColor: "rgba(248,113,113,0.6)", borderColor: "#f87171", borderWidth: 1 },
                ],
            },
            options: {
                responsive: true,
                ...
                scales: {
                    ...
                    y: {
                        grid: { color: "rgba(255,255,255,0.04)" },
                        ticks: { color: "#475569", font: { size: 10 } },
                        title: { display: true, text: "Litres", color: "#475569", font: { size: 10 } },
                    },
                },
            },
        });
    ...
    updateConsumptionChart(data) {
        ...
        const avg = arr => arr.length ? (sum(arr) / arr.length).toFixed(0) : "0";
        ...
        const totL = sum(loss);
        this.setText("periodLoss",  `${totS > 0 ? ((totL / totS) * 100).toFixed(1) : "0.0"}%`);
    }
    ...
    resetDisplayValues() {
        ["sourceFlowValue","destFlowValue","sourceTotalValue","destTotalValue",
         "flowDifferentialValue","literDifferenceValue","totalConsumptionValue",
         "lossPercentageValue"].forEach(id => this.setText(id, "-.-"));
    ...
    clearSensorReadings() {
        ["sourceFlowValue","destFlowValue","sourceTotalValue","destTotalValue",
         "flowDifferentialValue","literDifferenceValue","totalConsumptionValue",
         "lossPercentageValue","systemUptimeDisplay","systemUptime"]
            .forEach(id => this.setText(id, "-.-"));
        this.setText("systemStatusText", "OFFLINE");

        ["sourceStatus","destStatus","diffStatus"].forEach(id => {
            const el = document.getElementById(id);
            if (el) { el.textContent = "No Data"; el.className = "kpi-badge offline"; }
        });
    }
